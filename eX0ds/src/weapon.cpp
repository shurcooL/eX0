...
void CWeapon::Tick()
{
	...
	if ((bReloading || bShouldReload) || (false/*bAutoReload*/ && iClipAmmo <= 0 && fTimer <= 0
	  && true/*glfwGetMouseButton(GLFW_MOUSE_BUTTON_LEFT) == GLFW_RELEASE*/))
	{
		Reload();
	}

	if (true/*glfwGetMouseButton(GLFW_MOUSE_BUTTON_LEFT) == GLFW_RELEASE*/)
...
void CWeapon::GiveClip()
{
	...
	{
		if (false/*bAutoReload*/ && iClips == 0 && iClipAmmo == 0 && true/*glfwGetMouseButton(GLFW_MOUSE_BUTTON_LEFT) == GLFW_RELEASE*/)
		{
			// play buy ammo sound
			// ...
			iClips++;
...
void WeaponInitSpecs()
{
	...
	oWeaponSpecs[2].fROF = 1.5f;
	oWeaponSpecs[2].fReloadTime = 2.0f;
	oWeaponSpecs[2].fProjSpeed = 10;
	oWeaponSpecs[2].fInaccuracy = 0.0f;
	oWeaponSpecs[2].fMaxDamage = 25;
	oWeaponSpecs[2].fMaxLife = 3.0f;
}
