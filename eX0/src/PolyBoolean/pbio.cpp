...
local
void LoadPLINE2(FILE * f, PLINE2 **pline)
{
	*pline = NULL;
...
local
void LoadPAREA2(FILE *f, PAREA ** area)
{
    int paCount;
...
void LoadParea2(const char *szFname, PAREA **area)
{
    FILE * f = fopen(szFname, "r");
    if (f == NULL)
		error(err_io);
...
local
void SavePLINE2(FILE * f, const PLINE2 * pline)
{
    if (fprintf(f, "%d\n", pline->Count) < 0)
		error(err_io);
...
local
void SavePAREA2(FILE * f, const PAREA *area)
{
	int i = 0;
	const PAREA * pa = area;
    do {
...
void SaveParea2(const char *szFname, const PAREA * area)
{
	FILE * f = fopen(szFname, "w");
    if (f == NULL)
		error(err_io);
